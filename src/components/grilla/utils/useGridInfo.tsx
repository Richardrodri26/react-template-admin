...
import { IFilter, IMetaDataPagination, IPagination, IPaginationFilterDto, ISortType, ITheadGrid, signalType, useGridContext } from "../context"
...
const filterAdapter = (filt: IFilter[] | undefined, theadConfig: ITheadGrid[]) => {
    if (filt) {
        ...
        const filterGroupers = andFiltersGroupers.reduce((acumulator, current) => {
            let currentParam = current.param as string
            (acumulator[currentParam] = acumulator[currentParam] || []).push(current)
            return acumulator
...
export const useGetDefaultConfigByParams = () => {
    const { simpleSetFilter } = useGridContext()
    const [searchParams, setUrlParams] = useSearchParams()
    const filtToInsert = searchParams.get("filt")

    const [param, signal, value] = (filtToInsert ?? "").toString().split(",")

    let defaultOptionsPagination: IPagination = {
        filt: [],
        group: "",
        limit: "",
        page: "",
    ...
    useEffect(() => {
        ...
        if(filtToInsert) {
            simpleSetFilter(param, (signal as signalType), "text", value.replace('=', ''))

            searchParams.delete("filt")
