...
/**
 * change page when items count its zero, if it's the only page it doesn't change
 * @returns 
 */
export const useChangeGridPagination = () => {
    const { changePage, metaDataPagination } = useGridContext()

    const skipBlankPage = () => {
        if (metaDataPagination?.itemCount == 1) {
            changePage(metaDataPagination.currentPage == 1 ? 1 : metaDataPagination.currentPage - 1)
        }
    }

    return { skipBlankPage }
}